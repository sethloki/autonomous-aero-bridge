...
String NODE_ID = "B";  // CHANGE THIS for each node (e.g., B, C, D)

// ================== WIFI SETUP ==================
const char* ssid = "EMERGENCY_NETWORK"; // Changed to sound more official to survivors
...
uint8_t msgCounter = 0; // Short counter to save bytes

// Message memory (to avoid mesh infinite loops)
#define MAX_SEEN 20
...
String generateMsgID() {
  msgCounter++;
  // Creates a short, globally unique ID like "B1", "B2"
...
void sendNRF(String msg) {
  // CRITICAL SAFETY FIX: Force string to be under 32 bytes
  if (msg.length() > 31) {
...
void handleSend() {
  ...
  // Format: TYPE|ORIGIN|ID|TTL|DATA
  // TTL starts at 5 hops
  String message = "H|" + NODE_ID + "|" + msgID + "|5|" + coord;
  
  storeMsg(msgID); // Don't repeat our own generated message if it bounces back
  sendNRF(message);
...
void setup() {
  ...
  // Catch-all for Captive Portal pop-up
  server.onNotFound([]() {
...
// ================== LOOP ==================
void loop() {
  // Process Captive Portal & Web Traffic
  dnsServer.processNextRequest();
  server.handleClient();

  // ========== RECEIVE & FORWARD (THE MESH) ==========
  if (radio.available()) {
    char text[32] = "";
    ...
    // Mesh Deduplication: Ignore if we've seen this exact packet before
    ...
    // Forwarding logic
    if (ttl > 0) {
      // CSMA/CA Simulation: Wait a random amount of time so drones don't transmit over each other
      delay(random(20, 80)); 
  ...
  if (millis() - lastPing > 2000) {
    ...
    // Heartbeat TTL is 3 (doesn't need to bounce as far)
